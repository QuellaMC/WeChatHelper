...
*/
    ...
    private function baseText($postObj, $contentStr=''){
        if(empty($contentStr)){
            $options = Helper::options()->plugin('WeChatHelper');
            $contentStr =  '呜呜呜我还无法读取你发送的信息QwQ可以发送\'h\'来查看帮助！';
        }
        $fromUsername = $postObj->FromUserName;
        $toUsername = $postObj->ToUserName;
        $time = time();
	...
	private function chatText($postObj, $info)
    {
        if ($info != null) {
            $key        = $this->_tulingApi;
            $url        = "http://api.qingyunke.com/api.php?key=free&appid=0&msg=$info";
            $content    = file_get_contents($url);
            $content    = json_decode($content, true);
            $contentStr = $content['content'];
            $contentback = str_replace("{br}", "\n", $contentStr);
        }
        $fromUsername = $postObj->FromUserName;
        $toUsername   = $postObj->ToUserName;
        $time         = time();
        $resultStr    = sprintf($this->_textTpl, $fromUsername, $toUsername, $time, $contentback);
        return $resultStr;
    }
